...
from realsense import Camera #get_positions
...
def packed(grasp_type='top', positions=None):
    ...
    set_point(wall, Point(x=-0.4, y=0, z=1))
    ...
    for i in obj_dict:
        set_point(blocks[i], Point(x=obj_dict[i][0], y=obj_dict[i][1], z=block_height/2+0.001 ))
        set_euler(blocks[i], Euler(roll=0, pitch=0, yaw=math.pi/2 - obj_dict[i][2]/(180/math.pi)))

    choosed_blocks = blocks#random.sample(blocks, k=6)
    ...
    set_point(plate, Point(x=0.3, y=0.5, z=0))
    ...
    return Problem(robot=panda, movable=blocks, grasp_types=[grasp_type], surfaces=surfaces,
                   ...
                   goal_on=[(block, plate) for block in choosed_blocks])
