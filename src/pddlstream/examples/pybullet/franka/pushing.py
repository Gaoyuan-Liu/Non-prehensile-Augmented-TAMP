...
class Pusher:
    ...
    def push_core(self, robot, move_group, start_point, end_point, obj_poses): # 2D list
        ...
        self.ros_commander.execute_path(plan_waypoints)
    ...
    def push(self, robot, obj_poses, start, end):
        self.ros_commander.gripper_grasp()
        # Moveit
        move_group = moveit_commander.MoveGroupCommander("panda_arm")
        move_group.set_planner_id('BiRRTkConfigDefault')
...
if __name__ == '__main__':
    rospy.init_node('pusher')
    pusher = Pusher()
    
    pusher.push([0.4, 0.4], [0.4, -0.4])