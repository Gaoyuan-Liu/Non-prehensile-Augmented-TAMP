...
# Push
from examples.pybullet.panda.pushing import Pusher
from examples.pybullet.panda.scenario import scenario
# from camera import
...
def main(verbose=True):
    ...
    obj_poses = push(robot, obj, [0.6, 0.2], [0.6, -0.1])
    # camera()
    ...
    with HideOutput():
        ...
        problem = problem_fn(obj_poses=obj_poses, robot=robot) 
    ...
    with Profiler(field='tottime', num=25): # cumtime | tottime
        with LockRenderer(lock=not args.enable):
            solution = solve(pddlstream_problem, algorithm=args.algorithm, stream_info=stream_info,
                             ...
                             max_time=args.max_time, verbose=True, debug=False, visualize=True,
