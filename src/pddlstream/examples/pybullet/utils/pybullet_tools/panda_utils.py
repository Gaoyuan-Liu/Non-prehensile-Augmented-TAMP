...
TOP_HOLDING_LEFT_ARM = [0.0, 0.0, 0.0, -1.5, 0.0, 1.5, 0.0]
...
def get_groups():
    return sorted(PANDA_GROUPS)

def get_group_joints(robot, group):
    return joints_from_names(robot, PANDA_GROUPS[group])
...
def set_arm_conf(robot, joints, conf):
...
def get_gripper_joints(robot, arm):
    return get_group_joints(robot, 'hand')
...
SIDE_HEIGHT_OFFSET = 0.05 # z distance from top of object
HAND_LENGTH = 0.11
...
def get_top_grasps(body, under=False, tool_pose=TOOL_POSE, body_pose=unit_pose(),
    ...
    translate_z = Pose(point=[0, 0, h / 2 - grasp_length + HAND_LENGTH])
...
def create_gripper(robot, visual=True):
    link_name = 'panda_hand' #PR2_GRIPPER_ROOTS[arm]
