...
from examples.pybullet.franka.streams import get_cfree_approach_pose_test, get_cfree_pose_pose_test, get_cfree_traj_pose_test, \
...
from examples.pybullet.utils.pybullet_tools.utils import draw_base_limits, WorldSaver, has_gui, str_from_object, set_pose, \
    create_box, set_point, Point, remove_body, Euler, TAN, GREEN
...
# Push
# from examples.pybullet.franka.pushing import Pusher
from examples.pybullet.franka.scenario import scenario #, reset_obj_solvable
...
class pddlstream_solver():
    def __init__(self, robot):
        ...
        plate_length = 0.32
        plate_height = 0.03
        self.plate = create_box(plate_length, plate_width, plate_height, color=TAN)
        set_point(self.plate, Point(x=0.0, y=0.5, z=plate_height/2))
        # set_euler(self.plate, Euler(roll=0, pitch=0, yaw=math.pi/2 - self.plate_poses[0][2]/(180/math.pi)))
    ...
    def problem_formation(self, obj_poses, n_pick=None):
        ...
        for i in self.blocks:
            remove_body(i)
        print('Reset the block positions.')
        self.blocks = []
        for i in range(n_total):
            block_i = create_box(0.05, 0.05, 0.05, color=GREEN)
            self.blocks.append(block_i)

        #self.blocks = [create_box(0.05, 0.05, 0.05, color=GREEN) for _ in range(n_total)]
        grasp_type = 'top'

        obj_list = list(obj_poses)
        for i in range(n_total):
            set_point(self.blocks[i], Point(x=obj_poses[obj_list[i]][0], y=obj_poses[obj_list[i]][1], z=0.025))
            set_euler(self.blocks[i], Euler(roll=0, pitch=0, yaw=math.pi/2 - obj_poses[obj_list[i]][2]/(180/math.pi)))  # math.pi/2 - obj_dict[i][2]/(180/math.pi)
            # blocks.append(i)

        pick_blocks = random.sample(self.blocks, k=n_pick)
        ...
        # plate_length = 0.32
        # plate_height = 0.06
        # plate = create_box(plate_length, plate_width, plate_height, color=TAN)
        # set_point(plate, Point(x=self.plate_poses[0][0], y=self.plate_poses[0][1], z=plate_height/2))
        # set_euler(plate, Euler(roll=0, pitch=0, yaw=math.pi/2 - self.plate_poses[0][2]/(180/math.pi)))

        surfaces = [floor, self.plate]


        self.problem.robot = self.robot
        self.problem.movable = self.blocks
        self.problem.grasp_types = [grasp_type]
        self.problem.surfaces = surfaces
        # self.problem.goal_holding=[(arm, block) for block in blocks]
        self.problem.goal_on=[(block, self.plate) for block in pick_blocks]
    ...
    def pddlstream_from_problem(self, problem, base_limits=None, collisions=True, teleport=False):
    ...
    def problem_solve(self, obj_poses, n_pick):
        ...
        pddlstream_problem = self.pddlstream_from_problem(self.problem, collisions=True)
    ...
    def execution(self, problem, solution):
        ...
        if simulate:
            ...
            ros_commander.ros_commander(commands)
...
if __name__ == '__main__':
    ...
    # obj_poses = push(robot, obj_poses, [0.5, 0.2], [0.5, -0.1])
    # img2 = camera.observe()
    ...
    solver.solve_and_execute(obj_poses, 1)
