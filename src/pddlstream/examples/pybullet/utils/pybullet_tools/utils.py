...
BIN_URDF = file_path + '/../models/bin.urdf'
...
def connect(use_gui=True, shadows=True, color=None, width=None, height=None):
    ...
    #     disable_preview()
        ...
        # Set this in True, the camera will ingnore robot itself: 
...
def body_collision(body1, body2, **kwargs):
    # print(f"closest points = {get_closest_points(body1, body2, **kwargs)}")
...
def check_initial_end(start_conf, end_conf, collision_fn, verbose=True):
    ...
    # ans = collision_fn(start_conf, verbose=verbose)
    # print(f"collision = {ans}")
...
def joint_controller(body, joints, target, tolerance=1e-3, timeout=INF, **kwargs): # tolerance=1e-3
