...
GRASP_LENGTH = 0.02
APPROACH_DISTANCE = 0.05 + GRASP_LENGTH
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False):
    ...
    def fn(arm, obj, pose, grasp):
        ...
        # gripper_bias_quat = quaternion_from_euler(0, 0, -math.pi/4, axes='sxyz')
        # gripper_bias = ((0, 0, 0), tuple(gripper_bias_quat))
        # gripper_pose = multiply(gripper_pose, gripper_bias)
        ...
        grasp_conf = panda_inverse_kinematics(robot, arm, gripper_pose, custom_limits=custom_limits) #, upper_limits=USE_CURRENT)
...
def get_ik_ir_gen(problem, max_attempts=5, teleport=False, **kwargs):
    ...
    def gen(*inputs):
        ...
        while True:
            ...
            robot = problem.robot
            # base_joints = get_group_joints(robot, 'base')
            # base_conf = get_joint_positions(robot, base_joints) 
            # bq = Conf(robot, base_joints, base_conf)
            # print(f"inputs = {g.value}")
