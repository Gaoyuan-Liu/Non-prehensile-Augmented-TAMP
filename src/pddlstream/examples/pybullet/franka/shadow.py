...
class Shadow:
    def __init__(self, num = 8) -> None:
        ...
        # Arms
        # arm_dir = os.path.dirname(os.path.realpath(__file__)) + '/../../panda_gazebo/models/arm/meshes/'
        # print(arm_dir)
        # self.p0.setAdditionalSearchPath(arm_dir)
        # visualShapeId = self.p0.createVisualShape(shapeType=self.p0.GEOM_MESH,
        #                             fileName="arm.obj",
        #                             rgbaColor=[0, 0, 1, 1],
        ...
        #                             # meshScale=meshScale,
        #                             )
        # collisionShapeId = self.p0.createCollisionShape(shapeType=self.p0.GEOM_MESH,
        #                                         fileName="arm.obj",
        #                                         #   collisionFramePosition=shift,
        #                                         #   meshScale=meshScale
        #                                         )
        # self.arm1 = self.p0.createMultiBody(baseMass=1,
        ...
        # self.arm2 = self.p0.createMultiBody(baseMass=1,
        #               baseInertialFramePosition=[0, 0, 0],
        #               baseCollisionShapeIndex=collisionShapeId,
        #               baseVisualShapeIndex=visualShapeId,
        #               basePosition=[0.5, 0, 0.5],
    ...
    # def arms_collision_checking(self, robot_config, arm1_pose, arm2_pose):
    #     self.set_joint_angles(robot_config)
    #     self.set_object_pose(self.arm1, arm1_pose)
    #     self.set_object_pose(self.arm2, arm2_pose)


    #     self.p0.performCollisionDetection()

    #     contact_points = self.p0.getContactPoints(self.panda, self.arm1) + self.p0.getContactPoints(self.panda, self.arm2)
    ...
    def blocks_collision_checking(self, robot_config, obj_poses):
        ...
        ii = 0
        for i in obj_poses:
            # quat = obj_poses[i][i]#quaternion_from_euler(0,0,obj_poses[i][2])
            # pose = ((obj_poses[i][0], obj_poses[i][1], 0.025), quat)
            pose = obj_poses[i]
