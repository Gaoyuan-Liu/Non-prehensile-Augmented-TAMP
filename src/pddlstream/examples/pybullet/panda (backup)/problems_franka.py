...
from examples.pybullet.utils.pybullet_tools.utils import get_bodies, sample_placement, pairwise_collision, \
    add_data_path, load_pybullet, set_point, Point, create_box, stable_z, joint_from_name, get_point, wait_for_user, set_euler, Euler, \
...
import rospy
from examples.pybullet.panda.aruco_webcam import get_positions
...
def packed(grasp_type='top', num=1, positions=None):
    ...
    rospy.init_node("problems")
    positions, orientations = get_positions(n_blocks=num)

    for i in range(num):
        set_point(blocks[i], Point(x=positions[i][0], y=positions[i][1], z=block_height/2 ))
        set_euler(blocks[i], Euler(yaw=orientations[i][2]))



    # set_point(blocks[0], Point(x=positions[0][0], y=positions[0][1], z=block_height/2 ))
    # set_euler(blocks[0], Euler(yaw=orientations[0][2]))
    # set_point(blocks[1], Point(x=positions[1][0], y=positions[1][1], z=block_height/2 ))
    # set_euler(blocks[1], Euler(yaw=orientations[1][2]))
    # set_point(blocks[2], Point(x=positions[2][0], y=positions[2][1], z=block_height/2 ))
    # set_euler(blocks[2], Euler(yaw=orientations[2][2]))
    # set_point(blocks[1], Point(x=0.6, y=0.0, z=block_height/2 + block_height + 0.001 ))
    # set_point(blocks[2], Point(x=0.6, y=-0.0, z=block_height/2 + 2 * block_height + 0.002))
